...
import { API_URL } from "@/lib/config"; // Added API_URL import
...
export default function PostDetailModal({ post, onClose, onCommentAdded }: PostDetailModalProps) {
    ...
    useEffect(() => {
        ...
        const checkSaved = async () => {
            ...
            try {
                const res = await fetch(`${API_URL}/users/${userId}/saved/${post.id}/check`, {
    ...
    const handleToggleSave = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/users/${userId}/saved/${post.id}`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` }
            });
